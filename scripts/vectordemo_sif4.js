...
if (select == 4){
	...
	VectorSolver.setImageSmoothValue(1);
    VectorSolver.setSrcSmoothValue(1);
	VectorSolver.setUseMedianAprioriFilter(false);
	VectorSolver.setAPrioriVec(-0,1);
//	VectorSolver.setConstraintVec(1e5);
	VectorSolver.setConstraintVec(0.0005);
    VectorSolver.setConstraintSrcOET(1e-30);
    VectorSolver.setConstraintSrcTikhonov(0.001);
	VectorSolver.setSADiagonalvalue(0e-30);
	VectorSolver.setThermalTheshold(20);
	VectorSolver.setSrcVal(10);
	timestep1 = 5;
	timestep2 = 60;
	SADiagSecond = 0.00005;
	VecConstrainedSecond = 0.0005;
    ...
    maxCorridorLengthEnd = 0.5;
    maxCorridorLengthStart = 0.5;
