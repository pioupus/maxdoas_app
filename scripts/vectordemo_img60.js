...
if (select == 0){
	...
	VectorSolver.setConstraintVec(8e3);
    VectorSolver.setConstraintSrcOET(1e-19);
    VectorSolver.setConstraintSrcTikhonov(0.00005);
	VectorSolver.setSADiagonalvalue(0);
	VectorSolver.setThermalTheshold(280);
	...
	SADiagSecond = 100;
	VecConstrainedSecond = 2e-50;

	ConstraintSrcTikhonovSecond = 0.0005
}
if(select == 1){
	Filegroup = 2;
	SIGISSteps = 10;
		...
		if (ok){
			...
			for(iteration = 0;iteration < 2;iteration++){
				...
				if(iteration == 1){
					...
					VectorSolver.setConstraintSrcTikhonov(ConstraintSrcTikhonovSecond);
